...
- מצב מילה: חדר פרטי ומשחק מלא פועלים מקצה לקצה בשרת ובאפליקציה; משחק ברשת (Matchmaking) עדיין לא
...
- שרת Go: מנוע משחק, חדרים פרטיים, REST ו־WebSocket עם משחק פרטי מקצה לקצה, 6 קטגוריות תוכן ורשימת תגובות; Matchmaking עדיין לא
- Flutter: מעטפות Android/iOS; חדרים פרטיים ומשחקים מחוברים לשרת; משחק ברשת עדיין על נתוני דמה; הפונטים טרם נוספו
...
בדיקת ה־End-to-End מריצה ארבעה שחקנים במשחק פרטי מלא מול השרת:
...
### מה מחובר, מה דמה ומה פתוח

**מחובר לשרת:** כניסה כאורח ושמירת הזהות במכשיר, קטגוריות ותגובות מהשרת, יצירת חדר פרטי והצטרפות לפי קוד, לובי חי (הסרת שחקן, העברת ניהול, מנותקים), וכל שלבי המשחק בחדר פרטי: חשיפת תפקיד, רמזים לפי תור ושגיאות חסימה מהשרת, תגובות, הצבעה והצבעה חוזרת, ניחוש, תוצאה ומשחק נוסף. כל טיימר סופר לאחור ל־`deadline` של השרת. חיבור שנפל מתחבר מחדש אוטומטית עם הודעת `מתחברים מחדש...`, שחקן שהוצא רואה את מסך ההוצאה, ושרת שאיבד את ה־session מציג את מסך התקלה בלי הפסד ויוצר session חדש עם אותו כינוי ואווטאר.

**נתוני דמה (`app/lib/demo/`):** זרימת משחק ברשת (בחירת קטגוריות, חיפוש שחקנים ומשחק), כי ל־Matchmaking אין עדיין שרת. גם מסך `מצבי Prototype (debug)` במסך הבית משתמש בה.

**פתוח:** Matchmaking, שמירת ניצחונות והפסדים והגדרות, עריכת פרופיל, הפונטים Secular One ו־Rubik, מזהה אפליקציה סופי, שיתוף מערכת של קוד החדר, מילון תוכן לא ראוי (ובלעדיו משחק מתחיל רק בשרת עם `IMPOSTER_DEV_POLICY=1`), ותנאי שימוש ומדיניות פרטיות.
