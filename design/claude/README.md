...
הייצוא הנוכחי עדיין אינו כולל את מצב המשחק המקומי. העיצוב הבא צריך להרחיב אותו
לפי `docs/local-game-design.md` ו־`wireframes/local-game.html`, תוך שימוש בשני
הנכסים החדשים `local-one-device.webp` ו־`pass-the-device.webp`. אין למחוק או
לעצב מחדש את 29 המסכים הקיימים מעבר לעדכון מסך הבית ולהוספת מסך בחירת משחק ברשת.
