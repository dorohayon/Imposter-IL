...
- האפליקציה שולחת את ההוכחות של החנות ל־`POST /v1/entitlements` כשמשהו השתנה
  — מוצר נוסף או ירד, או מועד המנוי זז — ואחרי כל session חדש (כולל session
  שהוחלף אחרי אתחול שרת). שחזור שמוצא אותן רכישות אינו שולח דבר, כי StoreKit
  חותם מחדש את אותה עסקה בכל שחזור. חריג: מנוי ב־Play שומר על אותו token גם
  כשהוא מתחדש, ולכן הוא נשלח שוב כשהתקופה שהשרת מכיר מסתיימת בתוך יום — לכל
  היותר פעם בשעה עד שהחידוש מופיע — או כל 6 שעות, אם השרת לא נתן מועד. השליחה סדרתית: בקשה אחת בכל פעם, ולא יותר
  מאחת ממתינה.
...
- **משחק ברשת:** כל מחפש נבדק מול הקטגוריות שלו, בכל חיפוש — גם ב־`משחק נוסף`,
  כך שמנוי שפג או רכישה שהוחזרה מפסיקים מהמשחק הבא. הקבוצה משחקת בחיתוך
  הקטגוריות, ולכן שחקן חינמי מתאים רק לשחקנים שחולקים איתו קטגוריה חינמית.
- **תחילת משחק בחדר פרטי:** הקטגוריות נבדקות שוב. המנהל שמתחיל חייב להחזיק
  בהן, או שמי שבחר אותן עדיין בחדר ומחזיק בהן. החברים של הקונה משחקים בקטגוריות
  שלו כל עוד הוא שם; אחרי שעזב, או שהפרימיום שלו פג, מנהל חינמי לא ממשיך איתן.
...
- **הוכחה אחת פותחת כל session שמציג אותה.** בלי חשבונות אין זהות לקשור אליה
  רכישה: השרת בודק שההוכחה אמיתית, לא של מי היא. מי שמחלץ את ההוכחה שלו ומפיץ
  אותה מאפשר לאחרים, עם אפליקציה מתוקנת, לפתוח קטגוריות ברשת. מי שמתקן את
  האפליקציה פותח ממילא את הקטגוריות במכשיר, ולכן הבדיקה בשרת היא גדר נגד רמאות
  מזדמנת ולא כספת. הפתרון המלא הוא חשבונות. אם יתגלה ניצול: מגבלה בזיכרון על מספר
  ה־sessions הפעילים לכל הוכחה.
- **הוכחות Sandbox של Apple מתקבלות.** App Review קונה ב־Sandbox מול שרת
  הפרודקשן, בכל גרסה, ולכן דחייה שלהן הייתה מכשילה את הבדיקה. המחיר: רכישות
  החינם של בודקי TestFlight פותחות קטגוריות גם ברשת. סביבה שאינה `Production` או
  `Sandbox` (בדיקת StoreKit מקומית) נדחית.
...
שחזור רכישות נמצא גם בהגדרות, ולמנויים חודשיים יש שם גם `ניהול המנוי`, שפותח את
דף המנויים של App Store או של Google Play (שם מבטלים או משנים). שחקן פרימיום במכשיר חדש לא רואה אף קטגוריה נעולה,
...
| `in_app_purchase_storekit` | 0.4.13 | ה־JWS ב־`serverVerificationData`; שחזור דרך `Transaction.currentEntitlements`. StoreKit 2 דורש iOS 15, ולכן זו גרסת המינימום של האפליקציה |
