...
| `internal/api` | REST ו־WebSocket: sessions אורח (כינוי ואווטאר), יצירת חדר עם קוד ייחודי והצטרפות לפי קוד, חיבור אחד לכל session, idempotency, ping, טיימרי חדר ושליחת `session.state` ו־`room.state`. מחזיק הכול בזיכרון מאחורי מנעול אחד. |
