...
stateDiagram-v2
    [*] --> role_reveal
    role_reveal --> hints: כל המחוברים אישרו / 20 שניות
    hints --> hints: רמז נשלח / זמן תם / דילוג
    hints --> hint_break: רמז נשלח
    hint_break --> hints: 3 שניות
    hints --> pre_voting: כל התורות הסתיימו
    pre_voting --> voting: 5 שניות
    voting --> impostor_guess: המתחזה נבחר
    voting --> runoff_voting: תיקו
    voting --> hints: אזרח נבחר / אין קולות, אין הכרעה
    voting --> ended: שוויון כוחות / שתיקה שנייה
    runoff_voting --> impostor_guess: המתחזה נבחר
    runoff_voting --> hints: תיקו נוסף / אזרח נבחר / אין קולות, אין הכרעה
    runoff_voting --> ended: שוויון כוחות / שתיקה שנייה
...
| --- | --- | --- |
| `role_reveal` | 20 שניות | כל השחקנים הפעילים והמחוברים לחצו `הבנתי`, או שהזמן תם |
| `hints` | 60 שניות לתור (30/60/90 בחדר פרטי); 30 שניות אם השחקן מנותק | כל שחקן פעיל קיבל תור אחד בסדר אקראי |
| `hint_break` | 3 שניות | הזמן תם; הרמז שנשלח מוצג לפני התור הבא |
| `pre_voting` | 5 שניות | הזמן תם; הלוח המלא מוצג לפני ההצבעה |
| `voting` | 20 שניות, לא מסתיים מוקדם | הזמן תם |
| `tie_break` | 3 שניות | הזמן תם; התיקו מוכרז לכולם לפני ההצבעה החוזרת |
| `runoff_voting` | 15 שניות, רק השחקנים שבתיקו | הזמן תם |
| `impostor_guess` | 60 שניות, ניסיון אחד | ניחוש נשלח או הזמן תם |
| `ended` | — | — |

סיבות סיום (`EndReason`): `impostor_not_caught`, `impostor_parity`, `impostor_guessed_word`, `impostor_guess_wrong`, `impostor_guess_timeout`, `impostor_gone`, `not_enough_players`, `abandoned`.

תוצאה לכל שחקן: שחקן שיצא או הוצא — הפסד. שחקן שהודח בהצבעה — התוצאה של הקבוצה שלו. ניצחון אזרחים — כל האזרחים שנשארו ושהודחו מנצחים. ניצחון מתחזה — המתחזה מנצח והאזרחים מפסידים. `not_enough_players` — כל הנשארים מנצחים. `abandoned` — `none` לכולם, ואינו נרשם במכשיר.

### סבבים

המשחק ממשיך בסבבי רמזים־והצבעה עד שאחד הצדדים מנצח. `round` מתחיל ב־1 ועולה בכל סבב.

- **הודח אזרח:** מסומן `eliminated`, נשאר לצפות ולהגיב, בלי תור ובלי הצבעה. אם נשאר יותר מאזרח פעיל אחד — מתחיל סבב נוסף.
- **שוויון כוחות:** כשמספר המתחזים הפעילים שווה או גדול ממספר האזרחים הפעילים, המתחזה מנצח מיד (`impostor_parity`). עם מתחזה אחד זה קורה כשנשאר אזרח אחד.
- **הודח המתחזה:** עוברים לשלב הניחוש הקיים. ניחוש נכון — המתחזה מנצח; ניחוש שגוי או פקיעת זמן — האזרחים מנצחים.
- **תיקו:** הצבעה חוזרת בין השחקנים שקיבלו את מספר הקולות הגבוה; מצביעים כל הפעילים, אך רק הם יכולים לקבל קולות. תיקו נוסף, או סבב שאיש לא הצביע בו — איש לא מודח ומתחיל סבב נוסף.
- **הרמזים נשמרים** מכל הסבבים, מסומנים לפי `round`, ואיסור רמז כפול חל על המשחק כולו.
- **שחקן שיצא או הוצא:** נבדק אותו כלל שוויון כוחות. אזרחים שעוזבים עד שנשאר אחד הם ניצחון של המתחזה (`impostor_parity`), ולא `not_enough_players`.
- **שולחן שנטש:** שתי הצבעות ברצף שאיש לא הצביע בהן מבטלות את המשחק (`abandoned`). הראשונה היא סבב שהשולחן לא הכריע בו והמשחק ממשיך; השנייה מסיימת. הצבעה אחת תקפה מאפסת את הספירה, ותיקו אינו נחשב שתיקה — הצביעו, פשוט לא הסכימו. משחק שבוטל אינו נספר לאיש: `outcome` הוא `none`, לא ניצחון ולא הפסד.
...
- אם אף שחקן לא הצביע, מתחיל סבב חדש; בפעם השנייה ברצף המשחק מסתיים
  כ־`abandoned` ללא תוצאה.
- קול שנשלח נספר גם אם השחקן מנותק כשההצבעה נסגרת.
- אפשר להגיב לרמזים עד סוף המשחק.
- הצבעה חוזרת נמשכת 15 שניות.
- לחשיפת תפקיד יש 20 שניות; לאחר מכן המשחק ממשיך לרמזים גם אם לא כולם אישרו.
