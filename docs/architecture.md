...
stateDiagram-v2
    [*] --> role_reveal
    role_reveal --> hints: כל המחוברים אישרו
...
| --- | --- | --- |
| `role_reveal` | אין (ראו שאלות פתוחות) | כל השחקנים הפעילים והמחוברים לחצו `הבנתי` |
...
| `runoff_voting` | 20 שניות, רק השחקנים שבתיקו | הזמן תם |
...
stateDiagram-v2
    [*] --> writing: שחקן מחובר
    [*] --> awaiting_reconnect: שחקן מנותק (ניתוק +1)
...
## התנהגות זמנית שדורשת אישור

המנוע חייב להתנהג איכשהו במקרים שהאפיון לא מכריע. ההתנהגות הנוכחית מתועדת כאן ובבדיקות, והשאלות עצמן נוספו ל־[`open-decisions.md`](open-decisions.md#שאלות-שעלו-בתכנון-מנוע-המשחק):

| מקרה | התנהגות זמנית במנוע |
| --- | --- |
| טיימר לחשיפת תפקיד | אין טיימר; ממתינים לכל המחוברים. ניתן להגדרה ב־`RoleRevealTimeout`. |
| מה נחשב ניתוק | רק ניתוק כשהתור של השחקן פעיל. כל ניתוק כזה פותח 30 שניות ונספר מיד. |
| ניתוק שלישי | גם הוא מקבל 30 שניות; השחקן מוצא רק אם לא חזר. |
| חזרה בתוך 30 השניות | השחקן מקבל טיימר רמז מלא מחדש. |
| המתחזה יוצא או מוצא | המשחק מסתיים, המתחזה מפסיד וכל האזרחים שנשארו מנצחים. |
| אף שחקן לא הצביע | אף אחד לא נבחר, ולכן המתחזה מנצח. |
| קול של שחקן שמנותק בסוף ההצבעה | אינו נספר. |
| תגובות אחרי שלב הרמזים | מותרות עד סוף המשחק. |
| משך ההצבעה החוזרת | 20 שניות לפי חוקי המשחק (ה־Wireframe מציג 15). |
